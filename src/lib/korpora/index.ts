...
function shouldIncludeInTsv(header: string | null | undefined): boolean {
...
function buildTsvContent(
  ...
  for (let i = 0; i < rowCount; i++) {
    const row = selectedHeaders.map((header) =>
      sanitizeTsvValue(columns[header]?.[i])
    );
    lines.push(row.join("\t"));
  }
...
function writeTsvFile(slug: string, tsvContent: string): void {
...
function cssifyName(text: string): string {
  return text.replace(
    /[!\"#$%&'\(\)\*\+,\.\/:;<=>\?\@\[\\\]\^`\{\|\}~\s]/g,
    "_"
  );
...
async function processSheet(
  ...
  for (const i in langs) {
    ...
    const txt = meta.map((row: any) => row[lang]);
    columns[lang] = txt;
    ...
    buttons.push(
      ...
      select-none
      py-2 px-4
      ">${(languages as Record<string, { native?: string; short?: string; direction?: string }>)[lang]?.native ?? (languages as Record<string, { native?: string; short?: string; direction?: string }>)[prettifiedLang]?.native ?? prettifiedLang}</label>`
    );
  }
  table.push(`</tr>`);
  table.push(`</thead>`);
  ...
  const headers: Record<string, { header: string; priority: number; author: string; description: string }> = {};
  ...
  if (!ogImage) {
    ...
    if (fs.existsSync(pixraDir)) {
      ...
      const imageFiles = fs
        ...
        .filter((name) =>
          exts.has(path.extname(name).toLowerCase())
        )
        .sort();
...
async function processTitlesInParallel<T, R>(
  titles: T[],
...
(async () => {
  ...
  const processedData = await processTitlesInParallel(titles, async (title: string) => {
  ...
  await processTitlesInParallel(processedData, async ({ title, data }: { title: string; data: ProcessedSheetResult }) => {
