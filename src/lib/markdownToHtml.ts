...
const processor = unified().use(parse).use(slug).use(extractToc, { keys: ["data"], flatten: true });
...
export default async function markdownToHtml(markdown: string) {
  const result = await unified()
    ...
    .use(wrap, [
      ...
      { selector: 'div.md-typeset__table', wrapper: 'div.md-typeset__scrollwrap' }
    ])
    .use(link, {
      behavior: 'append', content: () => {
        return [
          h('span.in-heading.hash', '#'),
        ]
      }
    })
    .process(markdown)
  ...
  const tree = (processor as any).runSync(node).map((i: any) => ({ depth: i.depth, value: i.data.value, id: i.data.id }));
