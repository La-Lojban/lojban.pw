...
const Post = ({ post, preview }: Props) => {
  ...
  const toc_list: TocItem[] = (post?.toc.map((i: any) => ({ depth: i.depth, name: i.value, url: `${router.asPath.replace(/#.*/, '')}#${i.id}` })))
          ...
          {router.isFallback ? (
            ...
            <>
              <article className="mt-10 mx-auto max-w-7xl px-4 sm:mt-8 sm:px-6 flex md:flex-row flex-wrap">
                ...
                <PostBody post={post} />
                {has_toc && <nav className="hidden md:block toc w-full md:w-1/5 sticky px-2 bottom-0 md:top-20 h-16 md:h-screen font-medium text-sm overflow-ellipsis">
                  <div className="toc-core h-4/5 overflow-y-auto">
                    {toc_list.map((item) =>
                      <Link href={item.url} key={item.url}>
                        <a className={`block text-black in-toc hover:no-underline px-3 py-2 lme-ml-${(item.depth - 2) * 2}`}>
                          {item.name}
                        </a>
                      </Link>
                    )}
