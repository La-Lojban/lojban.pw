...
: Refers to one or more things that genuinely fit the description — things that would make a true _bridi_ with that relation as _selbri_. **lo** is like English _a/an_ or _some_ — you use it for real instances without singling out which ones.
...
### The five kinds of simple _sumti_
...
|---|------|---------------|---------------------|
| 1 | **Descriptions** | **le** / **lo** / **la** + _selbri_ (+ optional pieces) | This chapter; [relative clauses in Ch.11](/en/books/friendly-cll/11/) |
| 2 | **Pro-sumti** | **mi**, **do**, **ti**, **ko'a**, … | [Chapter 5](/en/books/friendly-cll/5/) |
| 3 | **Names** | **la** + _cmevla_ (or **la** + _selbri_ nickname) | [Chapter 5](/en/books/friendly-cll/5/); **la** + _selbri_ below in this chapter |
...
Complex _sumti_ add **relative clauses**, **sumti qualifiers** (**la'e**, **tu'a**, …), **abstractions**, and more — they still sit in the same argument slots; the subsequent sections of [Chapter 3](/en/books/friendly-cll/3/), [Chapter 11](/en/books/friendly-cll/11/) (relative clauses), and [Chapter 12](/en/books/friendly-cll/12/) (abstractions) cover each type.
...
> **pisu'o loi djacu cu blanu**
> _Some of the water (mass) is blue._

> **piro le'i prenu cu nanmu**
> _The entire set of people (I have in mind) consists of men._
...
In Chapter 2 you learned that putting two relation words side by side creates a _tanru_ where the first modifies the second. When you add a third word, Lojban's default is **left-grouping**: the leftmost two bind first.
...
For longer or more complex _tanru_, **ke** and **ke'e** work like parentheses: everything between **ke** and **ke'e** is treated as a single _tanru_ component.
...
**ke'e** can be omitted at the end of the _selbri_ if there's no ambiguity:
...
So far, **le** and **lo** treat their referents as individuals — even if there are multiple, each one individually satisfies the _bridi_.
...
Any of these can form a _tanru_ with a noun-like relation word:
...
(The _tanru_ **barda tcadu** = "big city" is used naturally throughout the story.)

---

### Sumti Qualifiers: Shifting What a Sumti Means

Sometimes you want to talk about something *associated with* a _sumti_ rather than the _sumti_ itself. Lojban provides **sumti qualifiers** that wrap around any _sumti_ and shift its reference:
...
: A loose _sumti_-raiser — refers to some event or fact related to the _sumti_.
...
Lojban's most "implicit" _sumti_ are the *indefinite* ones — _sumti_ that exist but don't need to be named:
...
This only works when there is no inner quantifier — just a plain number followed directly by the _brivla_. The meaning is always **lo**-like (genuine instances, veridical). This shorthand is common in spoken Lojban. You cannot drop **le** this way — bare numbers always imply **lo**.
...
A **sumti-based description** is one where a _sumti_ appears *in the slot normally filled by a _selbri_*. The inner quantifier is required and must be explicit.

The most common case: describe a *subset of a group you are already talking to or about*.

> **re do cu nanmu**
> _Two of you are men._ (of the listeners, size unknown, two are men)

> **le re do cu nanmu**
> _The two of you are men._ (I have a group of two listeners specifically in mind; all of them are men)

In **le re do**, the inner quantifier **re** gives the *size* of the group the inner _sumti_ **do** refers to. The implicit outer **ro** then says "all of those two."
...
> _The two-of-the-three bears are brown._ (a specific pair from that group of three)

> **pa le re le ci cribe cu bunre**
> _One of the two-of-the-three bears is brown._ (one specific bear from a specific pair from the original three)
...
Any _sumti_ placed between a descriptor and its _selbri_ works exactly like a **pe**-phrase. All the scoping and attachment rules for **pe** apply. This means:
...
### la + _selbri_: Names from Relation Words

**la** can be followed not just by a proper name but by any _selbri_:
...
The implicit outer quantifier for **la** + _selbri_ is **su'o** (at least one), just like **la** + proper name. This usage is common for giving descriptive nicknames or referring to characters in stories by their role names.

---

### Quotations and numbers (full chapters)

Simple-_sumti_ kinds **4** (quotations) and **5** (**li** / mekso) are only **named** early in this chapter; the teaching lives where they are used most:
...
- **Five kinds of simple _sumti_:** descriptions (**le**/…), pro-_sumti_, names, quotations, **li**-numbers — see table at the start of this chapter
...
- **tu'a** = something related to (vague _sumti_-raiser; paraphrase with **nu**/**du'u**/**ka** when precision matters)
...
- **le mi karce** = **le karce pe mi** — internal _sumti_ between descriptor and _selbri_ is a hidden **pe** phrase
...
- **Sumti-based descriptions** (**le re do**, **le re le ci cribe**): _sumti_ fills the selbri slot; inner quantifier required
