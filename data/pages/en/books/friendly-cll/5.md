...
> **lo smuci .i lo forca .i la rik. cu pilno ri xi re**
> _A spoon. A fork. Rick uses [the second-back = the fork]._

**ri xi re** skips one and picks up the next-to-last sumti; **ri xi mu** skips four and picks up the fifth-from-last. In practice this is cumbersome in speech — prefer assigning a ko'a slot for anything beyond a one-step back-reference.
...
- In **la .alis. cu klama le zarci .i ri barda**, **ri** = **la .alis.** (the most recent non-skipped sumti — actually **la .alis.** is a name and is counted! Here ri = le zarci since that is more recent). Names with **la** *are* counted by **ri**.
- Chaining: two **ri** in a row both refer to the *same* sumti — the second ri's antecedent is the first ri, which itself refers to the same underlying sumti.
