...
>
> *\^:i [!xa -jy]{.underline} \|vdr !xo -sto -fw -kaia  . . .*
>
...
:   I have *no* cheese. *\`\`xn\'\'* means that of the members of the
    ...
    *\`\`\^:i !ji [/go]{.underline} -kio !xa -kseo\'\'* \-\-- freely
...
:   I have *zero* pieces of cheese. This is the most natural form of
    ...
    set, it isn\'t a cheesy null set \-\-- there is only one null set.
...
:   This cheese is *bad* in flavor. In George Orwell\'s 1984, the
    ...
    *\`\`\^:i !jw \|kseo !fu [-zu -dyi]{.underline}\'\'* \-\-- \`\`this
...
members of a set, of the form \`\`S1 and S2 and S3 and . . .\'\', where
...
member, in form \`\`S1 or S2 or S3 or . . .\'\' When such statements are
...
:   I have at least one piece of cheese. Existential quantification like
    ...
    *or* . . .\'\' for all pieces of cheese. Now the simplest negation
...
:   I don\'t have any cheese \-\-- I don\'t have piece 1 *and* I don\'t
    have piece 2 *and* . . . To negate (or de-negate) a disjunction
...
*\^:i !xa -xe -cil /jir*
:   All the children are here \-\-- Child 1 is here *and* child 2 is
    here *and* . . . Rather than negating the whole sentence with
    \`\`*sfa*-false\'\', let us negate the predicate \`\`*jir*-here\'\':

*\^:i -go -jir !cil [\|zu -to]{.underline}*
:   At least one of the children is not here \-\-- Child 1 is not here
    *or* child 2 is not here *or* . . . In general, when you negate the
...
>
> Kira allows it to hold his hand (offers \-\-- but instead it swims
> away \-\-- infinitive not asserted)
>
...
:   So that rats cannot eat the cheese, is the *reason* the cheese is in
    ...
    *\`\`\^:i !xo -tara /gi -crw !kseo\'\'* = \`\`A rat might eat the
    ...
    \`\`vengeance for past event Y\'\' \-\-- a future consequence of
...
(ordered) consisting of members X2, X3, X4,  . . .\'\', as many cases as
