...
4. В чём выражается связь мышления с языком?
5. Для чего необходимо изучать логику?
...
2. ОПИСАНИЕ. Описание представляет собой перечисление ряда признаков единичного предмета, вида какого-либо животного или растения.
...
Другие примеры: «Пустыня — источник пыльных бурь», «Писатели — инженеры человеческих душ».
...
Это мы видим, например, в таком суждении: «Мой отец (есть) — строитель».
...
2. «Некоторые учителя известны на весь наш регион».
...
1. Если S есть Р, то S<sub>1</sub> есть P<sub>1</sub>. Например: «Если солнечный луч пропустить через призму, то на экране получится спектр».
2. Если S не Р, то S<sub>1</sub> не P<sub>1</sub>. Например: «Если ученик не проявит внимательности, то он не усвоит урока».
3. Если S есть Р, то S<sub>1</sub> не P<sub>1</sub>. Например: «Если через проволоку пропустить электрический ток, то химический состав её не изменится».
4. Если S не Р, то S<sub>1</sub> есть P<sub>1</sub>. Например: «Если картофель не окучивать, то урожай его будет низким».
...
Точно так же в суждении «Всякая деревня нашего региона находится или на равнине, или в горах, или в низине» очевидно, что отнесение данной деревни к нашему региону тем самым исключает отнесение его к какому-либо иному региону.

Иначе обстоит дело в суждении «Садовники или увеличивают полив некоторых растений, либо уменьшают количество растений». В данном суждении сказуемые не исключают друг друга, так как все факторы, о которых говорится в сказуемых суждения, могут действовать совместно. Суждения такого вида называются **соединительно-разделительными**.
...
Такие суждения называются **суждениями действительности** (ассерторические суждения). В них мы отображаем существующие в действительности связи предмета и свойства, фактическое положение вещей. Например, в суждении «Лондон расположен на Темзе» выражено действительное местоположение Лондона. Мы не мыслим при этом о закономерности такого явления, не имеем в виду его историческую обусловленность, мы только указываем на факт, хотя вообще мы знаем, конечно, что местоположение Лондона имеет свои причины.
...
Например: «Все тела в безвоздушном пространстве падают с одинаковой скоростью», «Сезоны года определяют поведение птиц», «Приход зимы неизбежен».
...
Так, в практике мышления часто приходится общее суждение преобразовывать в частное (О преобразовании суждений подробно будет говориться в главе VI). Например, общее суждение «Все металлы — элементы» можно преобразовать в частное суждение «Некоторые элементы — металлы».

Но суждение «Все металлы — элементы» нельзя преобразовать в суждение «Все элементы — металлы».

Почему? Потому что термин «элементы» в суждении «Все металлы — элементы» взят не во всём объёме, то есть не распределён. Поэтому в преобразованной форме суждения, когда термин «элементы» будет обозначать субъект суждения, этот термин нельзя брать во всём объёме.
...
Возьмём следующие предложения: «Эта земля — самая плодородная», «Это земля является самой плодородной», «Эта земля имеет такие свойства, что может давать самый хороший урожай».
...
1. Назовите существенные признаки понятий: «садоводство», «врачи», «колхозники», «клуб», «баня», «блог», «бизнес», «яблоко», «скульптура», «школа», «учебник».
2. Укажите объём и содержание следующих понятий: «фирма», «столица», «работники», «отличные работники».
...
5. В каких отношениях находятся понятия: «школьники» — «художники»; «яблоня» — «земля под яблочный сад»; «Лондон» — «Петербург»; «учёные» — «рабочие»; «станок» — «физический прибор»; «средняя школа» — «институт»; «день» — «ночь» — «сутки»; «поэма» — «проза»; «клавиша» — «клавиатура»; «яблоко» — «урожай яблок».
...
1. Дайте полный логический анализ следующего суждения:
...
13. «Некоторые школьники — не шахматисты; некоторые шахматисты — спортсмены». Какой вывод?
...
30. Суеверные люди считают, что «понедельник — тяжёлый день», что «рассыпать соль — значит поссориться» и пр. Какого рода логическую ошибку представляют эти суеверия?[](http://royallib.ru/author/vinogradov_s.html)
